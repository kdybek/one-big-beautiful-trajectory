...
class GCDataset:
    ...
    def sample_trajectories(self, n_traj):
        ...
        traj_end_idxs = self.terminal_locs[np.searchsorted(
            self.terminal_locs, traj_start_idxs)]

        trajs = []
        for start_idx, end_idx in zip(traj_start_idxs, traj_end_idxs):
            traj = self.dataset.get_subset(np.arange(start_idx, end_idx + 1))
            if self.config['frame_stack'] is not None:
                traj['observations'] = self.get_observations(
                    np.arange(start_idx, end_idx + 1))
                traj['next_observations'] = self.get_observations(
                    np.arange(start_idx + 1, end_idx + 2))
            trajs.append(traj)
...
@dataclasses.dataclass
class ACGCDataset(GCDataset):
    """Dataset for action-chunked goal-conditioned RL.

    Extends GCDataset so each sampled state returns a chunk of `action_chunk_length`
    consecutive actions instead of a single action.
    """
    ...
    def sample(self, batch_size, idxs=None, evaluation=False):
        """Sample a batch with 'action_chunks' of shape (batch_size, action_chunk_length, action_dim)
        in place of a single 'actions' entry. Goal sampling is inherited from GCDataset.
        """
        if idxs is None:
